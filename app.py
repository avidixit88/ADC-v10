...
.insight-card { border:1px solid rgba(255,255,255,.08); border-radius:18px; padding:16px 17px; min-height:118px; height:118px; background:linear-gradient(180deg, rgba(17,24,43,.78), rgba(8,12,24,.72)); overflow:hidden; }
.insight-label { color:var(--iv-muted); font-size:.76rem; text-transform:uppercase; letter-spacing:.12em; }
.insight-value { color:var(--iv-text); font-size:clamp(1.42rem, 1.75vw, 1.78rem); font-weight:850; letter-spacing:-.045em; margin-top:10px; white-space:nowrap; }
.insight-note { color:var(--iv-muted); font-size:.8rem; margin-top:5px; line-height:1.28; }
.future-calendar-wrap { max-height: 360px; }
.future-calendar-wrap .dark-table th { padding: 10px 12px; }
.future-calendar-wrap .dark-table td { padding: 10px 12px; vertical-align: middle; line-height:1.32; white-space: nowrap; }
.future-calendar-wrap .dark-table { table-layout:auto; }
...
with overview_tab:
    ...
    with c_e:
        note = "Later this year" if future_summary["later_this_year"] else "Future-dated estimates"
    ...
    with right:
        ...
        if not future_tbl_chart.empty:
            ...
            fig_f = px.bar(
                ...
                color_discrete_sequence=["#9d4dff", "#4f8cff", "#25d6b5", "#ff7a1a", "#ff5bd7", "#53e36b", "#d8b267", "#ff5f6d"],
            )
            fig_f.update_layout(
            ...
            st.plotly_chart(clean_plotly(fig_f, 305), use_container_width=True)
        else:
...
with trials_tab:
    ...
    cols = ["target", "nct_id", "brief_title", "overall_status", "phase_label", "phases", "start_date", "start_date_type", "primary_completion_date", "enrollment_count", "lead_sponsor", "collaborators", "conditions", "interventions", "countries", "site_count", "primary_outcomes"]
